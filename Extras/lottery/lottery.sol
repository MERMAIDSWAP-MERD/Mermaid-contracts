...
// File: contracts/interfaces/IPanmerdSwapLottery.sol

pragma solidity >=0.4.22 <0.9.0;

interface IPanmerdSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in MERD per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in MERD token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInMerd: price of a ticket in MERD
...
// File: contracts/PanmerdSwapLottery.sol

pragma solidity >=0.4.22 <0.9.0;
pragma abicoder v2;

/** @title PanmerdSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PanmerdSwapLottery is ReentrancyGuard, IPanmerdSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMerd = 50 ether;
    uint256 public minPriceTicketInMerd = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInMerd;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] merdPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMerd;
        uint32 finalNumber;
    }
     ...
     * @param _merdTokenAddress: address of the MERD token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _merdTokenAddress, address _randomGeneratorAddress) {
        merdToken = IERC20(_merdTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of MERD to this contract
        uint256 amountMerdToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMerd,
            _ticketNumbers.length
        );

        // Transfer merd tokens to this contract
        merdToken.safeTransferFrom(address(msg.sender), address(this), amountMerdToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMerd += amountMerdToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMerdToTransfer
        uint256 rewardInMerdToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMerdToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        merdToken.safeTransfer(msg.sender, rewardInMerdToTransfer);

        emit TicketsClaim(msg.sender, rewardInMerdToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMerd) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in MERD for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].merdPerBracket[j] =
                ...
                // A. No MERD to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].merdPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMerd - amountToShareToWinners);

        // Transfer MERD to treasury address
        merdToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        merdToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInMerd += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMerd >= minPriceTicketInMerd) && (_priceTicketInMerd <= maxPriceTicketInMerd),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMerd: _priceTicketInMerd,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            merdPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMerd: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(merdToken), "Cannot be MERD token");
    ...
    /**
     * @notice Set MERD price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInMerd: minimum price of a ticket in MERD
     * @param _maxPriceTicketInMerd: maximum price of a ticket in MERD
     */
    function setMinAndMaxTicketPriceInMerd(uint256 _minPriceTicketInMerd, uint256 _maxPriceTicketInMerd)
        external
        onlyOwner
    {
        require(_minPriceTicketInMerd <= _maxPriceTicketInMerd, "minPrice must be < maxPrice");

        minPriceTicketInMerd = _minPriceTicketInMerd;
        maxPriceTicketInMerd = _maxPriceTicketInMerd;
    }
     ...
     * @param _priceTicket price of a ticket (in MERD)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].merdPerBracket[_bracket];
        } else {
            return 0;
        }
    }
